...
public class NestedTest extends ParseTestBase {
    ...
    public void testС2() throws Exception {
        ...
        NgxBlock loc = TestUtils.assertBlock(conf.findBlock("http", "server", "location"), "location", "/hello");
        Iterator<NgxEntry> it = loc.iterator();
        TestUtils.assertParam(it.next(), "set", "$memc_cmd", "$arg_cmd");
        TestUtils.assertParam(it.next(), "set_unescape_uri", "$name", "$arg_name");
        TestUtils.assertParam(it.next(), "set_if_empty", "$name", "\"Anonymous\"");
        TestUtils.assertParam(it.next(), "memc_pass", "127.0.0.1:11211");
    }

    @Test
    public void testС5() throws Exception {
        NgxConfig conf = parse("nested/c5.conf");
        NgxBlock loc = TestUtils.assertBlock(conf.findBlock("http", "server", "location"), "location", "/foo");
        Iterator<NgxEntry> it = loc.iterator();
